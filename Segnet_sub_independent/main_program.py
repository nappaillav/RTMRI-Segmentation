...
directory='./pred_segnet/'+actual_set[0]+'/mask'+sys.argv[2];
...
if(new==1):
	print('*********new model Segnet fitting with Model_checkpoint*********')
	segnet_model=model.Segnet(n_classes ,68, 68)
	#segnet_model.summary()
	model_path = directory+'/model_'+actual_set[0];
	callbacks_list = [ModelCheckpoint(model_path, monitor='val_acc', verbose=0, save_best_only=True, mode='auto', period=1)]
	segnet_model.compile(loss='categorical_crossentropy',optimizer='adam',metrics=['accuracy'])
	history=segnet_model.fit(X_train, Y_train,epochs=30,batch_size=32,validation_data=(X_test,Y_test), callbacks=callbacks_list,verbose=1)
